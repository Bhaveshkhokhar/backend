...
exports.createOrder = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ order: order, confirmbooking: populatedConfirmBooking });
  } catch (error) {
...
exports.verifyPayment = async (req, res, next) => {
  ...
  try {
    ...
    if (expectedSignature === razorpay_signature) {
      ...
      const confirmbooking = await Booking.findByIdAndUpdate(order.booking_id, {
        paid: true,
      });
...
exports.retryPayment = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ order: order, booking_id: existingbooking._id });
  } catch (error) {
    console.error("Error while create order on rozar pay:", error);
    return res
      .status(500)
